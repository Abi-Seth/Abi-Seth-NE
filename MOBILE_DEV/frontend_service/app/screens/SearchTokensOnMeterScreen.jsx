...
import { buyToken, checkTokens } from '../services/token.service';

export default function SearchTokensOnMeterScreen({ navigation }) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            setLoading(true);
            setError("");
            const response = await checkTokens(values.meterNumber)
            ...
            if (response?.data.success) {
                ...
                setLoading(false)
                return setError(
                    response?.data?.message || "Error occurred while fetching"
                );
            }
        } catch (error) {
            setLoading(false);
                        ...
                        <View style={tw`mt-8`}>
                            <View style={tw`px-6 py-2`}>
                                ...
                                <View style={tw`mt-8`}>
                                    ...
                                    >
                                        {loading ? "Fetching tokens ..." : "Search Tokens"}
                                    </ButtonText>
                                </View>
                            </View>
                        </View>
